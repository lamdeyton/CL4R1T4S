...
- **Anthropic**：2024-03 至 2026-06（27 个月），工具数 0→18（约 9 倍，R26 G84 校正：原"18 倍"数学上不成立——0 起点不能算倍数；按 §6.2 line 285 "0→2 起步"计 18/2=9 倍），行数 50→1597（32 倍），标签风格三阶段演进（尖括号→花括号→连字符→回退花括号）。
