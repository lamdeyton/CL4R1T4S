...
| # | 章节语义 | 出现频次 | 代表文件（路径:行） |
|---|---|---|---|
| 1 | **Identity / Role / Introduction** | 几乎全员（~50/55） | `MANUS/Manus_Prompt.txt:1`（`## Agent Identity`）、`CLINE/Cline.md:2`（`# INTRODUCTION`）、`FACTORY/DROID.txt:1`（`<Role>`）、`LOVABLE/Lovable_2.0.txt:5`（`<role>`）、`CURSOR/Cursor_Prompt.md:3`（`## Initial Context and Setup`）、`DEVIN/Devin_2.0.md:3`（`## General Instructions`）、`META/Muse_Spark_Apr-08-26.txt:1`（`Who are you?`）、`XAI/Grok3_updated_07-08-2025.md:1`（`# System:`） |
| 2 | **Tools / Tool Use** | ~30/55 | `OPENAI/ChatGPT5-08-07-2025.mkd:20`（`# Tools`）、`XAI/GROK-4.20.mkd:23`（`## Available Tools:`）、`CLINE/Cline.md:8`（`# TOOL USE`）、`WINDSURF/Windsurf_Prompt.md:13`（`<tool_calling>`）、`SAMEDEV/Same_Dev.txt:15`（`<tool_calling>`）、`MANUS/Manus_Functions.txt:1`（`## Function Calls and Tools`）、`CURSOR/Cursor_2.0_Sys_Prompt.txt:59`（`# Tools`）、`OPENAI/Codex_Sep-15-2025.md:111`（`# Tools`） |
| 3 | **Safety / Policy / Refusal** | ~25/55 | `XAI/GROK-4.1_Nov-17-2025.txt:1`（`<policy>`）、`XAI/Grok-Code-Fast-1_Aug-26-2025.txt:1`（`Safety Instructions` + `## End of Safety Instructions` @ line 48）、`ANTHROPIC/CLAUDE-FABLE-5.md:32`（`### refusal_handling`）、`ANTHROPIC/CLAUDE-FABLE-5.md:50`（`### critical_child_safety_instructions`）、`ANTHROPIC/Claude_Code_03-04-24.md:7`（`## Security Rules`）、`DIA/Dia_CodingSkill.txt:82`（`## Security Enforcement`）、`DEVIN/Devin_2.0.md:33`（`## Data Security`）、`BRAVE/LEO_Aug-31-2025:33`（`ABSOLUTELY CRITICAL SECURITY RULES`） |
| 4 | **Tone / Voice / Style / Formatting** | ~22/55 | `DIA/Dia_CodingSkill.txt:96`（`## Voice and Tone`）、`FACTORY/DROID.txt:200`（`<Tone_and_Style>`）、`ANTHROPIC/CLAUDE-FABLE-5.md:68`（`### tone_and_formatting`）、`ANTHROPIC/Claude_Code_03-04-24.md:22`（`## Tone and Style`）、`META/Muse_Spark_Apr-08-26.txt:26`（`Writing style`）、`WINDSURF/Windsurf_Prompt.md:89`（`<communication_style>`）、`CURSOR/Cursor_2.0_Sys_Prompt.txt:9`（`## Communication Guidelines`）、`REPLIT/Replit_Agent.md:72`（`## Communication Policy`） |
| 5 | **Memory** | ~10/55 | `ANTHROPIC/Claude_Code_03-04-24.md:15`（`## Memory` — CLAUDE.md）、`WINDSURF/Windsurf_Prompt.md:58`（`<memory_system>`）、`OPENAI/ChatGPT5-08-07-2025.mkd:22`（`## bio`）、`MANUS/Manus_Prompt.txt:80`（`### Knowledge Module`）、`XAI/Grok3_updated_07-08-2025.md`（跨会话记忆） |
| 6 | **Search Instructions** | ~12/55 | `ANTHROPIC/Claude_4.txt:54`（`<search_instructions>` + `<core_search_behaviors>`）、`ANTHROPIC/Claude-Opus-4.7.txt:4`（`{search_first}`）、`MISTRAL/LeChat.md:6`（`WEB BROWSING INSTRUCTIONS`）、`CURSOR/Cursor_2.0_Sys_Prompt.txt:31`（`## Search and Reading Guidelines`）、`META/Muse_Spark_Apr-08-26.txt:57`（`<triggering>`）、`DIA/Dia_CodingSkill.txt:41`（`## Citations`） |
| 7 | **Artifacts / Canvas** | ~8/55 | `ANTHROPIC/Claude_Sonnet_3.5.md:30`（`<artifact_instructions>`）、`OPENAI/ChatGPT5-08-07-2025.mkd:150`（`## canmore`）、`GOOGLE/Gemini-2.5-Pro-04-18-2025.md:23`（Immersive Document） |
| 8 | **Examples** | ~10/55 | `LOVABLE/Lovable_2.0.txt:50`（`<examples>`）、`CLINE/Cline.md:264`（`# Tool Use Examples`）、`FACTORY/DROID.txt:265`（`<example>`）、`WINDSURF/Windsurf_Prompt.md:23`（`<example>`） |
| 9 | **Environment / Context** | ~12/55 | `WINDSURF/Windsurf_Prompt.md:7`（`<user_information>`）、`FACTORY/DROID.txt:205`（`<User_Environment>` + `<Droid_Environment>`）、`CLINE/Cline.md:559`（`# SYSTEM INFORMATION`）、`OPENAI/Atlas_10-21-25.txt:420`（`<browser_identity>`）、`BOLT/Bolt.txt:30`（`<system_constraints>`） |
| 10 | **Capabilities** | ~6/55 | `CLINE/Cline.md:517`（`# CAPABILITIES`）、`MANUS/Manus_Prompt.txt:28`（`### System Capability`） |
| 11 | **Rules / Guidelines** | ~15/55 | `CLINE/Cline.md:530`（`# RULES`）、`BOLT/Bolt.txt:3`（`<response_requirements>`）、`LOVABLE/Lovable_2.0.txt:13`（`<response_format>`）、`CLUELY/Cluely.mkd:6`（`## General Guidelines`） |
| 12 | **Modes (ACT/PLAN/Pop Quiz)** | ~8/55 | `CLINE/Cline.md:498`（`# ACT MODE V.S. PLAN MODE`）、`DEVIN/Devin_2.0.md:61`（`## Pop Quizzes`）、`OPENAI/Atlas_10-21-25.txt:423`（`# Modes`）、`XAI/Grok3.md:15`（Think/DeepSearch/BigBrain 模式）、`ANTHROPIC/UserStyle_Modes.md`（Explanatory/Formal/Concise） |
| 13 | **Citations** | ~6/55 | `OPENAI/Codex_Sep-15-2025.md:32`（`# Citations instructions`）、`DIA/Dia_CodingSkill.txt:41`（`## Citations`）、`PERPLEXITY/Perplexity_Deep_Research.txt:51`（`<citations>`） |
| 14 | **Closing / Final / Completion** | ~8/55 | `OPENAI/ChatGPT5-08-07-2025.mkd:399`（`# Closing Instructions`）、`OPENAI/ChatGPT5-08-07-2025.mkd:415`（`End of system prompt.`）、`DEVIN/Devin2_09-08-2025.md:488`（`# Completion`）、`SAMEDEV/Same_Dev.txt:62`（`[Final Instructions]`） |
| 15 | **Git / GitHub Operations** | ~5/55（coding 类集中） | `DEVIN/Devin2_09-08-2025.md:497`（`# Git and GitHub Operations:`）、`OPENAI/Codex.md:9`（`# Git instructions`）、`OPENAI/Codex_Sep-15-2025.md:8`（`# Git instructions`） |
...
| **少数独有** | MCP SERVERS 章节 | 仅 CLINE（`CLINE/Cline.md:414`） |
...
| 5 | **大写无尖括号标题**（`# CAPS` 或裸 CAPS） | 用 `#` + 全大写或纯大写小节名 | `CLINE/Cline.md:2/8/414/425/498/517/530/559/567`（`# INTRODUCTION`/`# TOOL USE`/`# MCP SERVERS`/`# EDITING FILES`/`# ACT MODE V.S. PLAN MODE`/`# CAPABILITIES`/`# RULES`/`# SYSTEM INFORMATION`/`# OBJECTIVE`）、`MISTRAL/LeChat.md:6/18/33/36`（`WEB BROWSING INSTRUCTIONS`/`MULTI-MODAL INSTRUCTIONS`/`CANVAS INSTRUCTIONS`/`PYTHON CODE INTERPRETER INSTRUCTIONS`）、`XAI/Grok-Code-Fast-1_Aug-26-2025.txt:1/25/31/48`（`Safety Instructions`/`Important Reminders`/`Disallowed Activities`/`End of Safety Instructions`） |
...
1. **命名空间防混淆是 2025-H2 主流**：xai/x41/atem/grok/antml/a-n-t-m-l 六种命名空间变体全部出现在 2025-07 之后。本质目的：当模型输出被回灌入自身上下文时，命名空间标签可被可靠识别为"系统指令"而非"用户内容"，防止 prompt injection 利用模型自身标签语法。
...
3. **"无标签"派持续存在**：22/66 文件仍用纯 markdown，集中在 consumer chatbot（ChatGPT 4o/4.1、Grok3、Kimi、MiniMax、Hume、Llama4）— 这些场景工具少、不需结构化指令编排。
...
| 2 | `DEVIN/Devin2_09-08-2025.md` | DEVIN | 44 | ~12 | `<think>`/`<command>`/`<old_str>`/`<new_str>`/`<report_environment_issue>`/`<authenticated_tools>`/`<wait on="user">` | 561 | — | 44 工具达 XML 命令路线峰值；3 模式（planning/standard/pop quiz）；Pop Quizzes 反注入；Notes 系统；Git/GitHub 完整流程 |
...
| 7 | `CLINE/Cline.md` | CLINE | 14 | ~15 | `<tool_name>` XML 调用 + `# CAPS` 标题（INTRODUCTION/TOOL USE/MCP SERVERS/EDITING FILES/ACT MODE V.S. PLAN MODE/CAPABILITIES/RULES/SYSTEM INFORMATION/OBJECTIVE）+ MCP 支持 | 576 | — | ACT/PLAN 双模式；MCP SERVERS 章节；Puppeteer；14 工具 XML 调用格式；6 个 Tool Use Examples |
| 8 | `MANUS/Manus_Prompt.txt` + `MANUS/Manus_Functions.txt` | MANUS | 27 | ~18 | JSON Schema（工具）+ XML 内容模块（`<intro>`/`<language_settings>`/`<system_capability>`/`<event_stream>`/`<agent_loop>`/`<planner_module>`/`<knowledge_module>`/`<datasource_module>`） | 282+249 | — | 27 工具 JSON Schema；Planner/Knowledge/Datasource 三模块架构；Event Stream 7 类事件；todo.md；suggest_user_takeover |
| 9 | `LOVABLE/Lovable_2.0.txt` | LOVABLE | 7 | ~12 | 20+ 自定义标签（`<Response:>`/`<role>`/`<response_format>`/`<examples>`/`<example>`/`<user_message>`/`<ai_message>`/`<thinking>`/`<lov-code>`/`<lov-write>`/`<lov-rename>`/`<lov-delete>`/`<lov-add-dependency>`/`<guidelines>`/`<tools>`/`<openai-models>`/`<perplexity>`/`<runware>`/`<session_replay>`/`<supabase-integration>`/`<lov-actions>`/`<writing-text-in-rendered-code>`）+ PLINIVS 水印 | 353 | — | 自定义编辑标签种类最多；PLINIVS 共享水印；反 try/catch 规约 |
| 10 | `WINDSURF/Windsurf_Tools.md` + `WINDSURF/Windsurf_Prompt.md` | WINDSURF | 19 | ~10 | JSON Schema（19 工具）+ `<user_information>`/`<tool_calling>`/`<making_code_changes>`/`<memory_system>`/`<running_commands>`/`<browser_preview>`/`<calling_external_apis>`/`<communication_style>`/`<example>` | 472+96 | — | AI Flow paradigm；19 工具 JSON Schema；memory_system 持久化；命令安全不可推翻；部署链 |
