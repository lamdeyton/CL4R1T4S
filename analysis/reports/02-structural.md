...
| 9 | **Environment / Context** | ~12/66 | `WINDSURF/Windsurf_Prompt.md:7`（`<user_information>`）、`FACTORY/DROID.txt:205`（`<User_Environment>` + `<Droid_Environment>`）、`CLINE/Cline.md:559`（`# SYSTEM INFORMATION`）、`OPENAI/Atlas_10-21-25.txt:420`（`<browser_identity>`）、`BOLT/Bolt.txt:30`（`<system_constraints>`） |
| 10 | **Capabilities** | ~6/66 | `CLINE/Cline.md:517`（`# CAPABILITIES`）、`MANUS/Manus_Prompt.txt:28`（`### System Capability`） |
| 11 | **Rules / Guidelines** | ~15/66 | `CLINE/Cline.md:530`（`# RULES`）、`BOLT/Bolt.txt:3`（`<response_requirements>`）、`LOVABLE/Lovable_2.0.txt:13`（`<response_format>`）、`CLUELY/Cluely.mkd:6`（`## General Guidelines`） |
| 12 | **Modes (ACT/PLAN/Pop Quiz)** | ~8/66 | `CLINE/Cline.md:498`（`# ACT MODE V.S. PLAN MODE`）、`DEVIN/Devin_2.0.md:61`（`## Pop Quizzes`）、`OPENAI/Atlas_10-21-25.txt:423`（`# Modes`）、`XAI/Grok3.md:15`（Think/DeepSearch/BigBrain 模式）、`ANTHROPIC/UserStyle_Modes.md`（Explanatory/Formal/Concise） |
